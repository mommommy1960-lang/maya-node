# Maya Node Founding Review — First 50 Customer Launch

## Objective

Sell and complete the first 50 $25 Maya Node Founding Reviews. The experiment tests whether a small, bounded consent-and-authority design review solves a problem people will pay to examine.

- Public overview: https://maya-node-founding-review.myqueen1960.chatgpt.site
- Secure checkout: https://buy.stripe.com/14A9AUc1A8bjeR75VE5wI00
- Fulfillment kit: [README.md](README.md)

## Best-fit first customers

Contact people already building or documenting:

- AI agents that can call tools, send messages, spend money, alter records, or publish externally;
- assistants with persistent memory or delegated authority;
- approval workflows where stale instructions or revoked consent could remain active;
- community, education, housing, or public-interest technology using sensitive decisions;
- early prototypes that need a plain-English authority map before deeper security or legal work.

Do not market the service as certification, compliance, penetration testing, legal advice, security approval, or proof that a product is safe.

## Plain-language offer

> Building an AI workflow that can remember, recommend, or act? The Maya Node Founding Review gives you a written map of who has authority, what the system may do, how permission expires or is revoked, and where the design may fail open. The first 50 reviews are $25 and include one written boundary-risk report, one practical adversarial test, and one clarification response.

## LinkedIn post

AI systems are often evaluated for what they can produce. They are not always examined with equal discipline for what they are allowed to do.

I have opened 50 founding places for a small, human-delivered Maya Node design review.

For $25, a participant receives:

- a written boundary-risk report;
- one practical adversarial test focused on a stated consent or authority limit; and
- one written clarification response.

This is not certification, legal advice, penetration testing, or a safety guarantee. It is a bounded second look at permission scope, expiration, revocation, audit evidence, and human override.

Overview: https://maya-node-founding-review.myqueen1960.chatgpt.site

Checkout: https://buy.stripe.com/14A9AUc1A8bjeR75VE5wI00

## Short-video script

“Your AI may be capable of doing something—but who actually authorized it?

I’m opening 50 $25 Maya Node Founding Reviews for builders working with AI agents, memory, tools, or approval workflows.

You receive a written boundary-risk report and one adversarial test aimed at consent, authority, revocation, or fail-closed behavior.

It is not certification or legal advice. It is a practical second look before a design becomes harder to correct.

The link is in the description.”

## YouTube description

Maya Node Founding Review — first 50 places, $25 each.

A bounded, human-delivered review of consent, permission scope, authority escalation, expiration, revocation, auditability, fail-closed behavior, and human override.

Includes one written boundary-risk report, one practical adversarial test, and one written clarification response.

Not certification, legal advice, penetration testing, a safety guarantee, or production approval.

Learn more: https://maya-node-founding-review.myqueen1960.chatgpt.site

Reserve: https://buy.stripe.com/14A9AUc1A8bjeR75VE5wI00

## Direct outreach template

**Subject:** A bounded consent-and-authority review for your AI workflow

Hello [Name],

I saw your work on [specific public project or feature]. Because it involves [specific memory, tool use, approval, or external-action boundary], I thought this small review might be relevant.

I have opened 50 founding places for the Maya Node Founding Review. It is a $25, human-delivered design review focused on consent, permission scope, expiration, revocation, audit evidence, fail-closed behavior, and human override.

The deliverable is one written boundary-risk report, one practical adversarial test, and one clarification response. It is not certification, legal advice, penetration testing, or a safety guarantee.

Overview: https://maya-node-founding-review.myqueen1960.chatgpt.site

If it is not relevant, no reply is needed.

Best,

Mya P. Brown  
Founder, The Commons Initiative

## Seven-day evidence experiment

| Day | Action | Evidence |
|---|---|---|
| 1 | Publish the LinkedIn post and one short video | URLs and timestamp |
| 2 | Contact up to five individually selected builders using specific public-project context | Count sent; no private addresses in repository |
| 3 | Publish one educational post explaining capability versus authority | URL and meaningful replies |
| 4 | Show one sanitized example authority map | URL and questions received |
| 5 | Contact up to five additional qualified builders | Count sent and response categories |
| 6 | Publish one adversarial-test example with invented data | URL and engagement |
| 7 | Record views, clicks, inquiries, completed checkouts, scope-qualified buyers, and objections | Aggregate results only |

## Decision rules

- **Continue unchanged:** at least one paid, scope-qualified review and delivery can be completed safely.
- **Revise message:** people engage but do not understand the offer.
- **Revise channel:** the offer is understood but reaches no relevant builders.
- **Revise price or deliverable:** qualified prospects identify the same concrete purchasing objection.
- **Stop and investigate:** checkout, intake, privacy, or fulfillment creates material harm or confusion.

Zero sales is evidence, not failure to record. Do not claim demand without completed purchases.
